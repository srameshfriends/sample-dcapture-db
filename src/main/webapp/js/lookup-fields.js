function LookupField(model, listener) {
    ...
    self.init = function () {
        ...
        if (typeof self.previousEle !== "undefined") {
            self.previousEle.on("click", function (evt) {
                ...
                evt.stopPropagation();
                if (self.isPrevious) {
                    let req = self.getRequest(self.offset - self.limit);
                    self.search(req);
                }
                return false;
            });
    ...
    self.setPaging = function (obj) {
        ...
        let offsetIdx = obj.offset + 1, endIdx = obj.offset + obj.length, currentCount;
        ...
        if (0 === obj.length && 0 === obj.totalRecords) {
            ...
            if (typeof self.infoEle !== "undefined") {
                if (0 === obj.length) {
                    ...
                    self.infoEle.text(offsetIdx + " - " + endIdx + " of " + obj.totalRecords);
                }
            }
        }
        if (0 === obj.offset) {
        ...
        currentCount = obj.offset + obj.length;
    ...
    self.getRequest = function (offset) {
        self.offset = 0 > offset ? 0 : offset;
        let req = {};
        req.name = model.name;
        req.offset = offset;
